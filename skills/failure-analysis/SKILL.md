...
6. Step 6 產出正式文件：在確認 `.md` 報告生成無誤後，請自主撰寫並於背景執行一段 Python 腳本（建議使用 `python-docx` 或 `pypandoc` 套件）。將剛剛生成的 Markdown 內容進行排版，並輸出為 `output/failure_analysis_report.docx` 供最終驗收。
